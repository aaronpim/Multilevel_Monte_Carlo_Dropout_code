...
from src.MLMC_base import make_config, MLMC_config, levels_to_num_evals, compute_mean_and_var, store_vals, expectation_of_estimator, variance_of_estimator
...
def single_level_multi_fidelity_from_evals(evals, fidelity_ladder, factor=None):
    running_mean = []
    running_var = []
    if factor is None:
        CONFIG = MLMC_config()
...
def estimator_from_evals(evals = None, model_seed = None, fidelity_ladder = [16, 32, 64], M = [43, 35, 25], factor = None):
    if evals is None:
...
def main_experiment(cost = 2048, fidelity_ladder = [16, 32, 64], model_seed = 0, num_seeds = 100,  factor = None):
    if factor is None:
        CONFIG = MLMC_config()
        factor = (CONFIG["eps_max"] - CONFIG["eps_min"])/(CONFIG["x_num"]*CONFIG["eps_num"])
