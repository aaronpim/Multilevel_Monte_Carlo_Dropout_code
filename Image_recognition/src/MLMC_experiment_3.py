...
from src.MLMC_base import make_config, multi_level_multi_fidelity_eval, levels_to_num_evals, compute_mean_and_var, store_vals, expectation_of_estimator, variance_of_estimator
from src.model_defn import load_model
from src.MLMC_experiment_2 import get_inputs_and_labels

def eval_models(total_cost = 2048, model_seed = 0, num_seeds = 100):
    with torch.no_grad():
        ...
        model.train()
        images, labels = get_inputs_and_labels()
        for i in range(num_seeds):
            set_seed(seed = i)
            output = torch.stack([model(images).squeeze() for _ in range(total_cost)])
            output = torch.softmax(output, dim = -1)
            output = output[:,torch.arange(output.shape[1]), labels]
            torch.save(output, f'plots/model_evals_{i}.pt')
            print(f'Completed seed {i+1}/{num_seeds}')

def single_level_multi_fidelity_from_evals(evals, fidelity_ladder):
    running_mean = []
    running_var = []
    for fid in fidelity_ladder:
        mean_est, var_est = compute_mean_and_var(evals[:fid])
...
def estimator_from_evals(evals = None, model_seed = None, fidelity_ladder = [16, 32, 64], M = [43, 35, 25]):
    if evals is None:
    ...
    for i, nrep in enumerate(num_levels):
        ...
        for _ in range(int(nrep)):
            ...
            Y0, dY, V0, dV = single_level_multi_fidelity_from_evals(block, fidelity_ladder[:i+1])
...
def main_experiment(cost = 2048, fidelity_ladder = [16, 32, 64], model_seed = 0, num_seeds = 100):
    ...
    for count, M in enumerate(all_possible_Ms):
        ...
        for i in range(num_seeds):
            ...
            _, mean_var, _, var_var, _, _ = estimator_from_evals(evals = evals, fidelity_ladder = fidelity_ladder, M = M)
