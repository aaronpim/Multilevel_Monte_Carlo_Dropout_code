...
def eval_models(total_cost = 1024, model_seed = 0, num_seeds = 100, downsize = 4):
    with torch.no_grad():
        ...
        for i in range(num_seeds):
            set_seed(seed = i)
            output = torch.stack([model(values).squeeze()[:, ::downsize, ::downsize, ::downsize] for _ in range(total_cost) ])
            torch.save(output, f'plots/model_evals_{i}.pt')
            print(f'Completed seed {i+1}/{num_seeds}, ETA: {((num_seeds/(i+1)-1)*(time.time()-start_time))//60} minutes')

def single_level_multi_fidelity_from_evals(evals = None, model_seed = None, fidelity_ladder= [16, 32, 64], weights= None, factor= (8/27)*(4**3)):
    if evals is None:
...
def estimator_from_evals(evals = None, model_seed = None, fidelity_ladder = [16, 32, 64], M = [22, 18, 12], weights = None, factor = (8/27)*(4**3)):
    if evals is None:
        if model_seed is None:
            evals =  torch.load(f'plots/model_evals_0.pt', weights_only = True)
        else:
    ...
    for i, nrep in enumerate(num_levels):
        ...
        for _ in range(int(nrep)):
            ...
            Y0, dY, V0, dV = single_level_multi_fidelity_from_evals(evals = block, fidelity_ladder = fidelity_ladder[:i+1], weights = weights, factor = factor)
...
def all_M(cost = 1024, fidelity_ladder = [16, 32, 64]):
...
def main_experiment(cost = 1024, fidelity_ladder = [16, 32, 64], model_seed = 0, num_seeds = 100,):
    ...
    for count, M in enumerate(all_possible_Ms):
        ...
        for i in range(num_seeds):
            ...
            _, mean_var, _, var_var, _, _ = estimator_from_evals(evals = evals, fidelity_ladder =fidelity_ladder, M = M, weights = weights, factor = factor)
...
def triangle_plot(M0, M1, data, file_name):
    ...
    m0_theo = 1024/(16*(1+np.sqrt(2))),
    m1_theo = 1024/(16*np.sqrt(2)*(1+np.sqrt(2)))
